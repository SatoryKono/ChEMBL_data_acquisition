...
## Локальный прогон тестов и проверка покрытия

Для регрессионной проверки перед изменениями запускайте unit- и smoke-тесты из
корня проекта:
...
Чтобы получить сводку покрытия по библиотечным модулям и скриптам CLI,
используйте `coverage` (пакет ставится вместе с dev-зависимостями):
...
В отчёте отображаются модули с проседанием покрытия. Особое внимание стоит
уделять `library/chembl_target.py`, `library/iuphar_library.py` и
`scripts/get_document_data.py`, поскольку они реализуют основную бизнес-логику
и требуют регулярного пополнения тестов при расширении функциональности.
