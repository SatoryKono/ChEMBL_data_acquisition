...
| `--no-reindex-raw` | Пропустить переупорядочивание колонок при сохранении «сырого» снимка и тем самым сохранить порядок, полученный от API. По умолчанию колонки сортируются по схеме; переключатель поддерживается в таргет-пайплайне и его обвязке. |
| `--normalize-at-export` / `--no-normalize-at-export` | Управляет финальной стадией нормализации. В таргет-пайплайне по умолчанию активен режим `--normalize-at-export`; укажите `--no-normalize-at-export`, чтобы сохранить необработанный payload и скопировать его в `--final-out`. Остальные скрипты пока игнорируют опцию до появления поэтапного вывода. |
...
В пайплайне таргетов `--raw-out` (совместно с `--raw-format parquet` при необходимости) сохраняет необработанный ответ, `--id-cols` удерживает составные ключи в этом снимке, `--no-reindex-raw` позволяет сохранить порядок колонок от API, а `--final-out` формирует нормализованную таблицу после валидации. По умолчанию включена нормализация (`--normalize-at-export`); укажите `--no-normalize-at-export`, чтобы оставить payload неизменным — в этом режиме CLI всё равно записывает «сырой» снимок и копирует его в `--final-out`, чтобы downstream-системы работали с тем же файлом. Устаревшие алиасы `--output`/`--out` пока остаются для совместимости, но сопровождаются предупреждениями. Если `--raw-out` не указан, этап выгрузки «сырых» данных пропускается. Остальные скрипты получат аналогичные стадии после обновления общего парсера.

> **Примечание.** Флаги `--raw-out`, `--final-out`, `--raw-format`, `--id-cols`, `--no-reindex-raw` и пара `--normalize-at-export` / `--no-normalize-at-export` реализованы в `get-target-data` и `library.utils.cli_tools.pipeline_targets_main`. Остальные CLI получат их после обновления общего интерфейса.
...
* Используйте `--offset` в любой подкоманде, чтобы пропустить идентификаторы, обработанные в предыдущем запуске.
* Комбинируйте `--raw-out` с `--no-reindex-raw`, если нужно сравнить снимок с ответом API без переупорядочивания колонок. Без переключателя порядок колонок приводится к схеме.
* Режим `--normalize-at-export` активен по умолчанию. Добавьте `--no-normalize-at-export`, чтобы сохранить необработанные данные: CLI запишет «сырой» снимок и скопирует его в `--final-out`, предоставив downstream-потребителям тот же payload. Явное указание `--normalize-at-export` помогает задокументировать ожидаемое поведение в автоматизированных сценариях.
* Отсутствие `--raw-out` отключает стадию «сырых» данных; при наличии опции YAML-метаданные фиксируют выбранный формат и факт переиндексации вне зависимости от режима нормализации.
