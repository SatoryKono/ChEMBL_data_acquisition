...
| `pipeline_skip_limit` | Записывается, когда `--limit 0` завершает работу до конфигурации, сети и файловой системы. |
...
Флаг `--limit 0` позволяет корректно пропустить пайплайн: появится событие
`pipeline_skip_limit`, а выполнение завершится до загрузки конфигурации,
сетевых вызовов и работы с файловой системой. Это удобно при оркестрации
многошаговых запусков.
...
* Значение `--limit 0` записывает `pipeline_skip_limit` и завершает работу до сетевых и файловых операций.
