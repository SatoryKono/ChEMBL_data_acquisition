...
4. **Finalization** — `finalise_targets` removes duplicates and entries lacking UniProt IDs, invokes the taxonomy classifier to derive `type` and related flags from UniProt lineage plus ChEMBL species data, lowercases selected fields, and `validate_and_write` performs normalization, adds metadata, validates against `TargetsSchema`, fills missing values with “-” while clearing placeholders in identifier columns, writes CSV with a fixed order, and emits a quality report.
