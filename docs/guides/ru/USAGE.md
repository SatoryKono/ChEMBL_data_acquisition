...
| `--final-out` | Основной путь финального экспорта для всех пайплайнов. По умолчанию формируется `output.<stem>_<YYYYMMDD>.csv` в `--output-dir` или `--base-path`. |
| `--output` / `--out` | Совместимые алиасы, которые перенаправляются в `--final-out` и сопровождаются предупреждением об устаревании. |
...
> **Примечание.** Флаги `--raw-out`, `--raw-format`, `--id-cols`, `--no-reindex-raw`, а также пара `--normalize-at-export` / `--no-normalize-at-export` реализованы в `get-target-data` и `library.utils.cli_tools.pipeline_targets_main`. Остальные CLI получат их после обновления общего интерфейса.
