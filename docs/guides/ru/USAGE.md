...
вспомогательные утилиты. Для английской версии обратитесь к `docs/guides/en/USAGE.md`.
...
| `--final-out` | Основной путь финального экспорта. По умолчанию формируется `output.<stem>_<YYYYMMDD>.csv` в `--output-dir` или `--base-path`. |
| `--output` / `--out` | Устаревшие флаги назначения, которые указывают на тот же путь, что и `--final-out`, и выводят предупреждение при каждом запуске. |
...
В пайплайне таргетов `--raw-out` (совместно с `--raw-format parquet` при необходимости) сохраняет необработанный ответ, `--id-cols` удерживает составные ключи в этом снимке, а `--final-out` формирует нормализованную таблицу после валидации. По умолчанию «сырой» дамп переупорядочивает столбцы по алфавиту; флаг `--no-reindex-raw` фиксирует порядок, пришедший от API. Пара `--normalize-at-export` / `--no-normalize-at-export` управляет тем, нормализуется ли итоговый CSV непосредственно перед записью (поведение по умолчанию) или копируется байт-в-байт из «сырого» снимка — при этом валидация всё равно выполняется по нормализованному представлению. Устаревшие алиасы `--output`/`--out` пока остаются для совместимости, но сопровождаются предупреждениями. Если `--raw-out` не указан, этап выгрузки «сырых» данных пропускается. Остальные скрипты получат аналогичные стадии после обновления общего парсера.
...
[`sources.chembl.molecule_catalog.cache_path`](../../reference/ru/CONFIG.md#sources-chembl-molecule-catalog); убедитесь,
...
`local.io.output_dir`). Полный список приведён в `docs/reference/ru/CONFIG.md`.
