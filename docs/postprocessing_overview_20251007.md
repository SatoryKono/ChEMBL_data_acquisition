...
* `_finalise_export` — центральный блок постобработки: строит DataFrame по `DocumentsSchema`, нормализует поля (`add_pipeline_metadata`, `dataframe_to_strings`), выполняет ленивую pandera-валидацию, сохраняет ошибки через `SidecarErrors`, вызывает `document_export_postprocessing.postprocess_export_file` для подготовки финальных файлов (`extended.output.document_*`). Также генерирует quality-отчёт (`finalise_csv_output`).【F:scripts/get_document_data.py†L360-L720】
...
| Постобработка экспорта | `document_export_postprocessing.postprocess_export_file` | итоговый CSV | `extended.output.document_*` | Создаёт расширенные выходы и фиксирует путь в логах. |
