...
| `library.utils.cli_tools.pipeline_targets_main` | `python -m library.utils.cli_tools.pipeline_targets_main --input targets.csv` | Запускает закешированную обвязку таргет-пайплайна для обновления артефактов и позволяет протестировать стадиальные флаги `--raw-out`, `--raw-format`, `--no-reindex-raw`, `--normalize-at-export` / `--no-normalize-at-export`. |
