...
- Flags: `--input`, `--final-out`, `--batch-size`, `--timeout`, `--limit`, `--offset`, `--pubchem-enable`/`--no-pubchem-enable`, `--postprocess`, `--config`. 【F:library/cli/commands/get_testitem_data.py†L891-L934】
