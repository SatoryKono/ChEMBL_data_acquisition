...
4. **Split configuration loading into a package**
   - **Description.** Break `library/config.py` into `library/config/loader.py`, `library/config/models.py` and `library/config/runtime.py`, each responsible for I/O, Pydantic models and session initialisation respectively.
