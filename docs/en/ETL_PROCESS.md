...
5. **Isoform expansion** — `library.postprocessing.target.process_targets` reads the exported table, expands `isoform_*` columns into per-isoform token rows, sorts them deterministically, and writes `isoform.output.<original-name>` (for example `isoform.output.targets_20250101.csv`) alongside the main export. The additional CSV is part of the [`OUTPUT_TARGETS`](./OUTPUT.md#target-export-targets) contract and is consumed by QA tooling for synonym coverage checks.
