...
5. **Organism lookup & isoform expansion** — `library.pipelines.target.postprocessing.finalise_file` emits the organism helper `organism.output.target_<stamp>.csv` with cellularity and multifunctional flags. `library.postprocessing.target.postprocess_target_table` mirrors the Power Query organism script, while `library.postprocessing.isoform.process_targets` reproduces the isoform expander to write `isoform.output.target_<stamp>.csv` (for example `isoform.output.target_20250101.csv`). Both artefacts are described in [`OUTPUT_TARGETS_EN.md`](../OUTPUT_TARGETS_EN.md) and feed the QA as well as activity enrichment steps.
