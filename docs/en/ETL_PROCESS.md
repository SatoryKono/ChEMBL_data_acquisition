...
5. **Organism lookup & isoform expansion** — `library.pipelines.target.postprocessing.finalise_file` emits the organism helper `organism.output.target_<stamp>.csv` with cellularity and multifunctional flags, while `library.postprocessing.target.process_targets` reproduces the Power Query isoform expander and writes `isoform.output.target_<stamp>.csv` (for example `isoform.output.target_20250101.csv`). Both artefacts are described in [`OUTPUT_TARGETS_EN.md`](../OUTPUT_TARGETS_EN.md) and feed the QA as well as activity enrichment steps.
