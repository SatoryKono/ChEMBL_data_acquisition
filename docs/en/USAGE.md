...
Every pipeline keeps the canonical deterministic bundle next to the final CSV:
...
`<stem>_failure_cases.csv` and related historical sidecars) are now opt-in to
reduce disk usage in long-running deployments. Enable them per run via
`--emit-legacy-artifacts`, `--debug` or `--keep-intermediate` whenever extended
investigation is required.
The first invocation after upgrading purges the outdated sidecars
automatically; rerun `tools/cleanup_legacy_outputs.py` to repeat the sweep or
perform a dry run.
...
logger. The ordered `steps` array summarises each pipeline stage after the
writer registers canonical artefacts and optional legacy sidecars. Every
successful step therefore contains:

- `output` — canonical path, existence flag, file size, SHA256 checksum plus
  `meta_path`/`meta_sha256` and `quality_path`/`quality_sha256` references when
  legacy artefacts are enabled.【F:library/reporting/run_manifest.py†L27-L166】【F:library/cli_utils.py†L682-L705】【F:library/cli_utils.py†L1158-L1299】
- `stats` — row counters and per-table diagnostics collected from the pipeline
  metrics. When metadata sidecars are emitted the same payload is embedded in
  `<name>.meta.yaml` for provenance.【F:library/common/metadata_writer.py†L69-L204】【F:library/cli_utils.py†L1158-L1299】
...
The example below shows a run executed with `--emit-legacy-artifacts`, hence the
optional metadata and quality paths are present in the manifest snapshot.
...
| `get-activities` | Generate synthetic activity rows and produce the canonical CSV + QA tables; add `--emit-legacy-artifacts` to persist `.meta.yaml`. |
