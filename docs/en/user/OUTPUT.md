...
`library.common.csv_utils.write_csv_deterministic`, which sorts rows and columns using
...
using `library.pipelines.common.add_pipeline_metadata`. The values are part of
...
`library.pipelines.document.pipeline.merge_metadata`. The following fields appear in the
CSV and schema:
...
built by `library.pipelines.document.pipeline.build_quality_report` and serialised via
...
`library.pipelines.target.pipeline.run_pipeline`, adds metadata and writes deterministic
