...
the export and executes `library.postprocessing.pipeline.targets.run_target_pipeline` via
`collect_postprocess_metrics`. The modular pipeline validates a compact schema
(`library/postprocessing/pipeline/targets/schema.py`) exposing `target_class`,
