...
* **Raw CSV / Parquet** — при наличии соответствующих флагов сохранение необработанного ответа в `--raw-out` (поддерживается `--raw-format parquet`).
...
* **Normalize** — унификация типов данных, операторов и регистра для детерминированной валидации.
...
> **Примечание.** Флаги `--raw-out`, `--final-out`, `--raw-format` и `--id-cols` доступны в `get-target-data` и `library.utils.cli_tools.pipeline_targets_main`. Остальные пайплайны получат их после расширения общего CLI.
