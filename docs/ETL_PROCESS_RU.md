...
* **Raw CSV / Parquet** — при наличии соответствующих флагов сохранение необработанного ответа в `--raw-out` (поддерживается `--raw-format parquet`). Добавляйте `--no-reindex-raw`, если нужно сохранить порядок колонок из API для отладки.
...
* **Финальный экспорт** — запись очищенной таблицы в `--final-out`/`--out`, добавление метаданных и отчётов качества.

> **Примечание.** Флаги `--raw-out`, `--final-out`, `--raw-format`, `--id-cols`, `--no-reindex-raw` и пара `--normalize-at-export` / `--no-normalize-at-export` доступны в `get-target-data` и `library.utils.cli_tools.pipeline_targets_main`. Остальные пайплайны получат их после расширения общего CLI.
