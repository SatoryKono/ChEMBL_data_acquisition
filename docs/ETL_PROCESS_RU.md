...
* **`library/constants/`** — определения `pandera`-схем и функции нормализации для каждой сущности.
