...
| Аспект | Детали |
| --- | --- |
| **Источники и формат входа** | CSV с колонкой идентификаторов (по умолчанию `activity_id`), читается лениво через `io.read_ids`. Дополнительные флаги `--limit` и `--dry-run` позволяют ограничить обработку.【F:scripts/get_activity_data.py†L78-L118】 |
