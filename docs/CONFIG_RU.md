...
|  | `chunk_size` | `5` | Размер батча при совмещении источников. |
|  | `sleep` | `5.0` | Пауза между циклами опроса (сек.). |
|  | `workers` | `1` | Количество потоков. |
|  | `batch_size` | `5` | Число ID в батче. |
|  | `timeout` | `30.0` | Таймаут запроса (сек.). |
...
* Аргументы, прокинутые через `apply_config_overrides`, обновляют конфигурацию. Например, `--chunk-size 25` задаёт `sources.chembl.pipelines.activity.chunk_size` на время запуска.
