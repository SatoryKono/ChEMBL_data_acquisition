...
- Финализация таргетов теперь читает тип организма напрямую из
  автоматической таблицы `dictionary/_Target/targets_type.csv`. Наследуемый
  справочник организмов удалён из поставки. При необходимости используйте
  переопределение `local.resources.targets_type_csv` для собственных
  таблиц.【F:library/target_postprocessing.py†L485-L608】【F:scripts/get_target_data.py†L1008-L1031】
- Подкоманда `get_target_data.py all` принимает флаг `--targets-type-csv`
  вместо прежнего `--organism-csv`. Обновите скрипты автоматизации, если старый
  параметр передавался явно.【F:scripts/get_target_data.py†L312-L371】
