...
- При локальном обновлении словарей без коммита добавляйте новые хэши через
  переменную окружения ``CHEMBL_DICTIONARY_EXTRA_HASHES``, например
  ``{"dictionary_root": ["<sha256>"]}``. Значение ``"*"`` временно отключает
  проверку контрольных сумм для отладки.
