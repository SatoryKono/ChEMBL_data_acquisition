...
Конфигурация проверена с зависимостями в следующих диапазонах (см. `pyproject.toml`):
...
| `workers` | `1` | Количество параллельных рабочих процессов для обработки батчей API. |
| `limit` | `null` | Ограничение на число идентификаторов. |
| `dry_run` | `false` | Пропуск сетевых вызовов и записи файлов; оркестратор пишет `step_skip_dry_run` и не создаёт временные/сигнальные файлы. |
...
Пути вида `data/input/ChEMBL/*.xlsx` являются заглушками для локальных проверок. Перед запуском процедур инициализации замените их на книги, подготовленные вашей организацией (или разместите вручную выданные файлы в нужном каталоге). Подробности подготовки входных данных приведены в [docs/ru/user/USAGE.md](../user/USAGE.md).

## Системные настройки (`system`)

| Подсекция | Ключ | Значение | Описание |
| --- | --- | --- | --- |
| `log` | `level` | `INFO` | Уровень логирования по умолчанию. Структурированный JSON-вывод использует фиксированную схему из [`library/common/log.py`](../library/common/log.py) и [`library/common/logging_setup.py`](../library/common/logging_setup.py), поэтому формат сообщения и таймстемпа через конфигурацию не меняется. |
