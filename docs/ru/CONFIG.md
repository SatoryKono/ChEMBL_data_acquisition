...
| `batch_size` | `25` | Размер пакета. |
| `timeout` | `60.0` | Таймаут. |
| `limit` | `null` | Ограничение записей. |

> **Примечание.** При локальном переопределении в `config.local.yaml`
> проверяйте соответствие `batch_size` и `timeout` ограничениям API.
...
Переменная окружения `CHEMBL_DA_RUN_ID` задаёт значение по умолчанию для
идентификатора запуска в `library.cli.parser.create_logger_config`. Параметр
`--run-id` в командной строке имеет приоритет над автоматически вычисленным или
переданным через окружение значением.
