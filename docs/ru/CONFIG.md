...
1. Значения по умолчанию, определённые в [`library/config.py`](../library/config.py).
...
короткие алиасы (см. `_ALIAS_MAP` в `library/config.py`).
...
| `timeout_read` | Нет | `30` | Таймаут чтения (сек). |
...
| `batch_retry.enable` | `false` | Включить уменьшение батча при ошибках. |
...
Подробности и дополнительные поля см. в [`library/config.py`](../library/config.py);
