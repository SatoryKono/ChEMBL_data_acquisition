...
`data/logs/<script>_<YYYYMMDD>.log` в корне репозитория. Переменная
`CHEMBL_DA_BASE_PATH` переносит директорию в `<base>/logs`, при этом относительные
пути интерпретируются относительно корня репозитория. Формат записей остаётся
`[время] [уровень] [логгер] сообщение`, что упрощает последующую диагностику.
...
Необходимо явно указать режим через `--mode <chembl|pubmed|all>` или,
для обратной совместимости, передать позиционную команду `chembl`, `pubmed`
или `all`. При отсутствии параметра CLI завершится с ошибкой; чтобы выполнить
объединённый прогон, выберите `--mode all` (или позиционную команду `all`). Флаг
заменяет прежние позиционные подкоманды, сохраняя общие аргументы CLI в духе
остальных пайплайнов.
...
Ниже режимы указаны явно — это обязательное требование. Используйте флаг
`--mode` или позиционный аналог в зависимости от сценария.
