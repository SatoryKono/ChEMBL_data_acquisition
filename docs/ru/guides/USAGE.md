...
- `--rerun-postprocess` — пересобирает согласованные с этапами выгрузки даже при
  наличии прошлых запусков, что удобно после правок нормализации при
  сохранении кешированных стадий.
- `--debug` — включает расширенную диагностику и сохраняет промежуточные
  артефакты для всех дочерних пайплайнов.
- `--keep-intermediate` — сохраняет промежуточные и диагностические каталоги без
