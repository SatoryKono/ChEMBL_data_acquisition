...
- `get_document_data` по умолчанию выполняет режим `all`; задавайте `--mode`,
  если нужен конкретный этап. `get_target_data` по-прежнему требует явную
  подкоманду (`chembl`, `uniprot`, `iuphar`, `all`).
