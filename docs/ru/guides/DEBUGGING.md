...
- Для `get_document_data` обязательно указывать `--mode`, а для
  `get_target_data` — подкоманду (`chembl`, `uniprot`, `iuphar`, `all`).
...
При повторяющихся ошибках соберите логи (`<base>/logs/*.log`, по умолчанию
`<base>` — каталог `data`), файл метаданных и проблемные строки CSV, затем
создайте issue с описанием уже проделанных шагов.
