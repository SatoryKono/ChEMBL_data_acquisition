...
После каждого запуска также формируется `reports/run_manifest.json` относительно
`--base-path`. В манифесте фиксируются все шаги: путь к целевому CSV,
обнаруженные sidecar-файлы, статус (`success`, `skipped`, `failed` и т.д.),
длительность и SHA256-суммы артефактов. Файл создаётся даже при частичном
выполнении, что позволяет проанализировать промежуточные результаты.
...
- Базовые флаги (`--input`, `--final-out` и т.д.).
...
таймаутов, режима dry-run и параллельной загрузки. Помимо общих флагов доступны
опции:
...
схеме. Дополнительные параметры:
