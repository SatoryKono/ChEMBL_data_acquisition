...
| `--no-reindex-raw` | Сохраняет порядок колонок, полученный от API, вместо алфавитного переупорядочивания «сырого» снимка. Доступен вместе с `--raw-out`. |
...
| `--id-cols` | Список колонок идентификаторов, передаваемый раздельными аргументами. Укажите каждое имя после флага (например, `--id-cols target_chembl_id pref_name`), чтобы сохранить столбцы в «сыром» снимке до очистки. Значения синхронизируются с конфигурацией, если заданы как YAML-список. Доступно в таргет-пайплайне и его вспомогательной обвязке. |
| `--normalize-at-export` / `--no-normalize-at-export` | Булева пара, управляющая нормализацией перед записью финального CSV (по умолчанию включена). Отключение сохраняет итоговый файл байт-в-байт равным «сырому» снимку, при этом валидация выполняется по нормализованному представлению. |
...
В пайплайне таргетов `--raw-out` (совместно с `--raw-format parquet` при необходимости) сохраняет необработанный ответ, `--id-cols` удерживает составные ключи в этом снимке, а `--final-out` формирует нормализованную таблицу после валидации. Имена колонок передаются отдельными аргументами после флага (например, `--id-cols target_chembl_id pref_name`), чтобы обеспечить детерминированный порядок ключей. По умолчанию «сырой» дамп переупорядочивает столбцы по алфавиту; флаг `--no-reindex-raw` фиксирует порядок, пришедший от API. Пара `--normalize-at-export` / `--no-normalize-at-export` управляет тем, нормализуется ли итоговый CSV непосредственно перед записью (поведение по умолчанию) или копируется байт-в-байт из «сырого» снимка — при этом валидация всё равно выполняется по нормализованному представлению. Устаревшие алиасы `--output`/`--out` пока остаются для совместимости, но сопровождаются предупреждениями. Если `--raw-out` не указан, этап выгрузки «сырых» данных пропускается. Остальные скрипты получат аналогичные стадии после обновления общего парсера.
...
Тот же список можно задать в конфигурации (например, `id_cols: [target_chembl_id, pref_name]`); CLI-переопределения продолжат синхронизироваться с настройками через `apply_config_overrides`.

> **Примечание.** Флаги `--raw-out`, `--final-out`, `--raw-format`, `--id-cols`, `--no-reindex-raw`, а также пара `--normalize-at-export` / `--no-normalize-at-export` реализованы в `get-target-data` и `library.utils.cli_tools.pipeline_targets_main`. Остальные CLI получат их после обновления общего интерфейса.
...
* Используйте `--offset` в любой подкоманде, чтобы пропустить уже обработанные идентификаторы.
* Флаг `--no-reindex-raw` сохраняет порядок колонок из API в «сыром» снимке; по умолчанию столбцы сортируются по алфавиту для стабильности.
* `--normalize-at-export` активен по умолчанию, чтобы финальный CSV соответствовал очищенной схеме. Укажите `--no-normalize-at-export`, если потребитель ожидает точную копию «сырого» дампа (валидация по-прежнему использует нормализованный вид, а метаданные фиксируют обе версии).
...
также поддерживает все стадийные флаги (`--raw-out`, `--raw-format`,
`--id-cols`, `--no-reindex-raw`, `--normalize-at-export` / `--no-normalize-at-export`),
поэтому можно протестировать разделение «сырого» и нормализованного вывода без
обращений к внешним API. Подходит для проверки переопределений конфигурации,
логирования и параметров батчирования до запуска `get_target_data` с обращениями
к внешним сервисам.
