...
- `--rerun-postprocess` — пересобирает согласованные с этапами выгрузки даже при наличии прошлых запусков, чтобы обновить постобработку.
- `--debug` — включает расширенную диагностику и сохраняет промежуточные артефакты для всех запускаемых пайплайнов.
- `--keep-intermediate` — сохраняет промежуточные и диагностические артефакты без обязательного включения `--debug`.
...
|-------|------------------------|------------|
| `--mode` | `all` (по умолчанию) | Выбор сценария. |
...
Если флаг `--mode` не передан, конвейер автоматически запускает объединённый сценарий `all`, сохраняя привычное поведение прошлых версий.
...
Явный выбор режима не обязателен — запуск без `--mode` уже выполняет объединённый
прогон `all`.
