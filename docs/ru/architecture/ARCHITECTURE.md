...
Оркестратор инициализирует общую конфигурацию, логирование, лимитеры и по
очереди вызывает CLI для документов, таргетов, ассайев, тестовых объектов и
активностей. Пайплайн тканей запускается отдельно, когда требуется подготовить
данные для объединения с активностями. Внутри используются общие компоненты
`library/`:
...
Оркестратор выполняет эти пять скриптов последовательно, если отдельные этапы
не отключены флагами CLI. Пайплайн тканей остаётся отдельной CLI-командой для
ручных обогащений.
