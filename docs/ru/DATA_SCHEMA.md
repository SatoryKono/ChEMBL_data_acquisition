...
### activity.csv
- **Назначение:** исходный список идентификаторов активностей ChEMBL, который читает CLI `get_activity_data` перед обращением к API.
...
### assay.csv
- **Назначение:** перечень идентификаторов биологических тестов ChEMBL, которые обрабатываются скриптом `get_assay_data`.
...
### documents.csv
- **Назначение:** список идентификаторов документов ChEMBL, являющийся входом для `get_document_data` (режимы `chembl`/`all`).
...
### targets.csv
- **Назначение:** входной набор идентификаторов целей ChEMBL для пайплайна `get_target_data`.
...
### testitem.csv
- **Назначение:** перечень идентификаторов молекул ChEMBL, которые обогащаются структурной и PubChem информацией в `get_testitem_data`.
...
- **Назначение:** нормализованный набор экспериментальных значений активности, полученный через ChEMBL API, дополненный вычисленными границами, структурированными аннотациями и метаданными пайплайна.
...
| lower_value | число (float) | Производное поле | Консолидированная нижняя оценка, вычисленная по границам ChEMBL, операторам и метаданным об ошибке.|
| upper_value | число (float) | Производное поле | Консолидированная верхняя оценка, вычисленная по границам ChEMBL, операторам и метаданным об ошибке.|
| activity_properties | строка (JSON) | Производное поле | Канонический JSON с описанием измерения, контекста теста и доп.флагов.|
| action_type | строка | Производное поле | Нормализованная метка действия (например, `PAM`, `NAM`), полученная из аннотаций и словарей.|
| properties_hash | строка | Производное поле | SHA-256 контрольная сумма `activity_properties` для отслеживания изменений.|
...
- **Назначение:** агрегированный набор описаний биологических тестов с подсчётом числа тестов на ту же пару документ/мишень и метаданными пайплайна.
...
| sortorder.target | строка | Классификатор таксономии | Сортировочный ключ для аналитических витрин, рассчитанный классификатором.|
...
- **Назначение:** описание лекарственных веществ/соединений из ChEMBL, дополненное иерархией родитель→соль, структурными полями, данными PubChem, признаками каталога и метаданными пайплайна.

| Колонка | Тип данных | Источник | Описание |
| --- | --- | --- | --- |
| molecule_chembl_id | строка | ChEMBL `/molecule` | Основной идентификатор молекулы.|
| parent_molecule_chembl_id | строка | Иерархия ChEMBL/каталог | Родительский идентификатор, используемый для агрегации солей.|
| salt_chembl_id | строка | Производное поле | Содержит идентификатор дочерней записи, если молекула — соль; иначе пусто или `-` по настройке.|
| natural_product | булево (nullable) | Каталог молекул | Нормализованный признак природного происхождения.|
| prodrug | булево (nullable) | Каталог молекул | Нормализованный признак пролекарства.|
| polymer_flag | булево (nullable) | Каталог молекул | Нормализованный признак полимерности.|
