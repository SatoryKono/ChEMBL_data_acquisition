...
### 4.1 Оркестратор `get-data` (консольная команда)

- Запускается через консольную команду `get-data` (`library.cli.entrypoints:get_data_main`). Совместимая обёртка `python scripts/get_data.py` оставлена для автоматизации, но прокидывает только общие флаги этапов и не формирует манифесты или расширенные переопределения.
...
- Базовые флаги позволяют настроить пути, логирование и перезапуски без изменения канонического плана:
  ...
  | `--force-pubchem` | Принудительно включить PubChem, даже если YAML его отключает. | Обойти временные флаги конфигурации во время расследований. |
  ...
  Флаги разбираются `_parse_args` и нормализуются в `PipelineRunConfig`, сохраняя порядок шагов и валидацию.【F:library/cli/commands/get_data.py†L949-L1108】 Манифест запуска формируется только при вызове консольной команды.
- Расширенные переопределения меняют граф исполнения и используются точечно. Они доступны исключительно в `get-data`:
...
## 8. Экспорт DOCX для внешнего распространения

Утверждённый DOCX формируется перед публикацией, поэтому в Git хранится только
Markdown-источник. После проверки наличия `pandoc` в `PATH` выполните
конвертацию:
...
Скрипт валидирует входы, при необходимости добавляет нумерацию и оглавление и
создаёт релизный артефакт без добавления его в репозиторий. Процедура включена
в чек-лист распространения документации в
`documentation_update_report.md`. Для русскоязычного варианта укажите в качестве
входа `docs/ru/PROTOCOL_RU.md` и скорректируйте имя выходного файла по
необходимости.【F:scripts/convert_md_to_docx.py†L15-L128】【F:docs/documentation_update_report.md†L1-L13】
