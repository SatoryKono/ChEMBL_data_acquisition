...
5. **Развёртка изоформ** — `library.postprocessing.target.process_targets` читает экспортированную таблицу, разворачивает колонки `isoform_*` в строки по изоформам и токенам, сортирует результат детерминированно и записывает файл `isoform.output.<исходное-имя>` (например, `isoform.output.targets_20250101.csv`) рядом с основным экспортом. Дополнительный CSV входит в контракт [`OUTPUT_TARGETS`](../en/OUTPUT.md#target-export-targets) и используется QA-утилитами для проверки покрытия синонимов.
