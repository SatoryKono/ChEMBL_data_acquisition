...
def test_targets_type_csv_default_from_config(tmp_path, monkeypatch) -> None:
    """``targets_type_csv`` argument should default to the config value."""
    ...
    def fake_run_all(cfg, args) -> int:  # type: ignore[unused-argument]
        captured["targets_type_csv"] = args.targets_type_csv
        return 0

    monkeypatch.setattr(gtd, "run_all", fake_run_all)
    rc = gtd.main(["all", "--config", str(cfg_path)])
    assert rc == 0
    assert captured["targets_type_csv"] == Path("dictionary/targets_type.csv")
