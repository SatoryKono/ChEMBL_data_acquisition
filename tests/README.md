...
The repository-level `pytest.ini` keeps detailed duration reporting but now filters out `slow` and `network` markers by default to avoid lengthy runs in CI and local quick checks. Use `pytest -m "slow or network"` (or pass the same selector after `--` when calling `scripts/run_tests.py`) to explicitly include the heavier suites. If you still prefer fail-fast behaviour locally, append `-x` or `--maxfail=<N>` to your command; the global configuration no longer forces it so that CI can collect the full failure set for triage.
