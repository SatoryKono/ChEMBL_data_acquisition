...
@pytest.mark.unit
def test_ensure_output_directories__cleans_existing_outputs(
...
def test_main__handles_json_report_write_failure(
    ...
    def _failing_write_json(report: dict[str, Any], destination: Path) -> None:
        nonlocal write_attempts
        write_attempts += 1
        raise OSError("simulated json write failure")

    monkeypatch.setattr(run_tests, "write_json_report", _failing_write_json)

    summary_attempts = 0

    def _fake_write_summary(report: dict[str, Any], destination: Path) -> None:
        nonlocal summary_attempts
        summary_attempts += 1
        destination.write_text("# summary\n", encoding="utf-8")

    monkeypatch.setattr(run_tests, "write_summary", _fake_write_summary)
