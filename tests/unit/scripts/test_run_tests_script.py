...
@pytest.mark.unit
def test_build_structured_report__no_tests_failure_sets_zero_success_rate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(run_tests, "_git_output", lambda *args, **kwargs: "stub")

    structured = run_tests.build_structured_report({"tests": []}, exit_code=2)

    assert structured["summary"]["success_rate"] == 0.0
    assert structured["meta"]["exit_code"] == 2


@pytest.mark.unit
def test_build_failure_report__captures_reason(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(run_tests, "_git_output", lambda *args, **kwargs: "stub")

    report = run_tests.build_failure_report(3, reason="pytest crashed")

    assert report["meta"]["exit_code"] == 3
    assert report["summary"]["success_rate"] == 0.0
    assert report["summary"]["total"] == 0
    assert report["tests"] == []
    assert report["meta"].get("failure_reason") == "pytest crashed"
...
@pytest.mark.unit
def test_main__produces_failure_reports_when_pytest_crashes(
    ...
    assert "Success rate: 0.00%" in summary_text
    assert captured_configs and captured_configs[-1].level == "INFO"


@pytest.mark.unit
def test_main__falls_back_to_failure_report_when_generation_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    reports_dir = tmp_path / "reports"
    coverage_dir = reports_dir / "coverage"
    ...
    def _boom() -> dict[str, Any]:
        raise RuntimeError("raw report missing")

    monkeypatch.setattr(run_tests, "_load_raw_report", _boom)
    ...
    payload = json.loads(report_file.read_text(encoding="utf-8"))
    assert payload["summary"]["success_rate"] == 0.0
    assert payload["meta"].get("failure_reason") == "raw report missing"
    summary_text = summary_file.read_text(encoding="utf-8")
    assert "Success rate: 0.00%" in summary_text
    assert captured_configs and captured_configs[-1].level == "INFO"
