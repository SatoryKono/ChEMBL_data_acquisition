...
from library.pipelines.common import ChunkedFetchConfig, CsvWriterConfig, PipelineRunResult
...
class _LoggerStub:
    ...
    def exception(self, message: str, *args, **kwargs) -> None:  # pragma: no cover - helper
...
def test_run_activity_pipeline__successful_execution(monkeypatch, cfg, tmp_path) -> None:
    fetch_config = ChunkedFetchConfig(ids=["A1", "A2"], chunk_size=2, workers=1)
    writer_config = CsvWriterConfig(writer=lambda *_args, **_kwargs: tmp_path / "out.csv", kwargs={})
    ...
    monkeypatch.setattr(activity_run, "prepare_chunked_pipeline", fake_prepare_chunked_pipeline)
...
def test_run_activity_pipeline__failure_result(monkeypatch, cfg, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: (lambda: (), lambda *a, **k: Path(k.get("destination", tmp_path / "out.csv"))),
    )
    ...
    tracker = activity_run.ChunkFailureTracker()
    save_calls: list[tuple[Path, object]] = []
    monkeypatch.setattr(tracker, "save", lambda path, *, cfg: save_calls.append((Path(path), cfg)), raising=False)
