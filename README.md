...
| Tissue | `python scripts/get_tissue_data.py --input data/input/tissue.csv --final-out output/tissues.csv --batch-size 25 --limit 100 --offset 10 --timeout 45` | Retrieves tissue metadata from the ChEMBL `/tissue` endpoint, honours batching/limit controls and writes a deterministic lookup table. |
