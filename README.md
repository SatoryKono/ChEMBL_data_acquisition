...
* Командные скрипты с унифицированными флагами `--input`, `--final-out`
  ...
  сохраняются для совместимости и сопровождаются предупреждением при каждом
  запуске),
...
PY
  ...
  EN: In the reporting example above `--final-out` sets the destination. The
  legacy `--output`/`--out` aliases remain available for compatibility but now
  raise deprecation warnings on every run.

  RU: В примере с отчётностью путь задаёт `--final-out`. Устаревшие алиасы
  `--output`/`--out` сохраняются для совместимости, но при каждом запуске
  выводят предупреждение об устаревании.
...
options like ``--input``, ``--final-out`` (primary destination flag across the
tooling) and ``--limit``. The compatibility aliases ``--output``/``--out`` stay
available for now but trigger explicit deprecation warnings. Passing ``--limit
0`` short-circuits processing before any network or filesystem access, which is
handy for configuration smoke tests. The target pipeline already exposes
``--raw-out``, ``--final-out``, ``--raw-format`` and ``--id-cols``; other
commands will gain the staging switches once the shared CLI is extended.
...
now raise a deprecation warning whenever they are invoked. When ``local.io.exist_ok`` is set to ``false``
the directory has to exist beforehand; otherwise it is created automatically. The target pipeline uses the
additional ``--raw-out``/``--final-out`` switches to separate staging outputs.
