...
| Test item | `python scripts/get_testitem_data.py --input data/input/testitem.csv --final-out output/testitems.csv --batch-size 250 --pubchem-enable --postprocess` | Enables parent-molecule enrichment with PubChem toggles (`--pubchem-enable`/`--no-pubchem-enable`) and deterministic post-processing (`--postprocess`/`--no-postprocess`) that merges hierarchy lookups and QA artefacts. |
