...
def process_ids(
    ...
    *,
    ...
    """Process a list of UniProt IDs and write extracted data to ``output_csv``.
    ...
    ----------
    uniprot_ids:
        An iterable of UniProt accession identifiers.
    output_csv:
        Destination path for the output CSV file.
    data_dir:
        Directory where JSON files for each ID are stored.
    sep:
        Field delimiter used for the output CSV file. Defaults to a comma.
    encoding:
        File encoding for the output CSV file. Defaults to UTF-8.
...
def process(
    ...
    *,
    ...
    None
        The processed information is written to ``output_csv``.
    """
    ids = list(iter_ids(input_csv, sep=sep, encoding=encoding))
    process_ids(
        ids, output_csv, data_dir=data_dir, sep=sep, encoding=encoding
    )


def get_info_for_ids(
    uniprot_ids: Iterable[str], data_dir: str = "uniprot"
) -> pd.DataFrame:
    """Return a DataFrame with UniProt information for each ID.

    This function collects the same information as :func:`process_ids`
    but returns it as a pandas DataFrame instead of writing to a CSV.
    ...
    -------
    pd.DataFrame
        A DataFrame containing the collected UniProt information.
    """
    records = []
    for uid in uniprot_ids:
        info = collect_info(uid, data_dir)
