...
class IUPHARData:
    ...
    def map_uniprot_df(self, df: pd.DataFrame) -> pd.DataFrame:
        """Map UniProt IDs to classification data in a DataFrame.

        This method applies the same logic as :meth:`map_uniprot_file` but
        operates on an in-memory DataFrame.

        Parameters
        ----------
        df:
            DataFrame with a ``uniprot_id`` column.

        Returns
        -------
        pandas.DataFrame
            DataFrame with added IUPHAR classification columns.
        """
        if "uniprot_id" not in df.columns:
            raise ValueError("Input DataFrame must contain 'uniprot_id' column")
