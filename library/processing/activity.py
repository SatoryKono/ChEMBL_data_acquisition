"""Activity processing utilities used by enrichment pipelines."""
...
from ..config import ActivityActionTypeCfg, ActivityBoundsCfg, ActivityPropertiesCfg
...
_POSITIVE_KEYWORDS = {"positive allosteric", "pam"}
_NEGATIVE_KEYWORDS = {"negative allosteric", "nam"}
...
_NONNEGATIVE_TYPE_EXCLUSIONS = {"delta", "dg", "enthalpy", "gibbs", "log"}
  ...
  measurement = _clean_structure(
    {
      ...
      "units": _first_non_empty([record.get("standard_units"), record.get("units")]),
...
def annotate_action_properties(
  df: pd.DataFrame,
  action_cfg: ActivityActionTypeCfg,
  properties_cfg: ActivityPropertiesCfg,
) -> pd.DataFrame:
  """Backward compatible alias for :func:`apply_activity_annotations`."""

  return apply_activity_annotations(df, action_cfg, properties_cfg)
...
def compute_activity_bounds(
  ...
  def _to_numeric(column: str) -> pd.Series:
    if column not in df:
      return pd.Series(np.nan, index=df.index, dtype="float64")
    return pd.to_numeric(df[column], errors="coerce")

  value = _to_numeric("standard_value")
  lower = _to_numeric("lower_value")
  upper = _to_numeric("upper_value")

  relation = df.get("standard_relation", df.get("relation", pd.Series(dtype="object")))
  relation = relation.fillna(df.get("relation")) if "relation" in df else relation
  ...
  relation_normalised = relation.map(_normalize_relation)

  def _parse_uncertainty(value_str: str | None) -> tuple[float | None, float | None]:
    if not value_str:
      return None, None
    match = _UNCERTAINTY_PATTERN.match(value_str)
    if not match:
      return None, None
    base = float(match.group("value"))
  ...
  for idx, row in df.iterrows():
    relation_token = relation_normalised.iloc[idx]
    if relation_token == "between" and pd.notna(lower.iloc[idx]) and pd.notna(upper.iloc[idx]):
      continue
    if relation_token == ">=" and pd.notna(value.iloc[idx]):
      bounds_lower.iloc[idx] = value.iloc[idx]
    elif relation_token == "<=" and pd.notna(value.iloc[idx]):
      bounds_upper.iloc[idx] = value.iloc[idx]
    elif relation_token == "~":
      low, high = _parse_uncertainty(str(row.get("standard_value")))
      if low is not None and high is not None:
        bounds_lower.iloc[idx] = low
        bounds_upper.iloc[idx] = high

  result = df.copy()
  if cfg.lower_column:
    result[cfg.lower_column] = bounds_lower
  if cfg.upper_column:
    result[cfg.upper_column] = bounds_upper
  return result
