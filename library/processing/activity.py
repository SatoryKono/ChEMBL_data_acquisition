"""Activity processing helpers for ChEMBL data acquisition."""
...
class ActivityFrameResult:
    """Container for the fetched activity frame and metadata."""
...
class ActivityWriteResult:
    """Information about generated activity outputs."""
...
class ActivityProcessingError(RuntimeError):
    """Base class for errors raised during activity processing."""


class ActivityReadError(ActivityProcessingError):
    """Raised when identifiers cannot be read from the input CSV."""
...
class ActivityFetchError(ActivityProcessingError):
    """Raised when ChEMBL activity data cannot be retrieved."""
...
def _apply_uncertainty_bounds(
    ...
    matches = text.astype(str).str.strip().str.extract(_UNCERTAINTY_PATTERN)
    if matches.empty:
        return lower, upper
...
def fetch_activity_frame(
    cfg: Config,
    input_csv: Path,
    *,
    limit: int | None = None,
) -> ActivityFrameResult:
    """Fetch and enrich the activity frame using *cfg* settings."""

    with ChemblClient(cfg.api, cfg.retry, cfg.chembl) as client:
        try:
            ids_iter = io.read_ids(input_csv, column=cfg.activity.column, cfg=cfg.io)
        except (FileNotFoundError, ValueError) as exc:  # pragma: no cover - defensive
            raise ActivityReadError(input_csv, exc) from exc
        ...
        try:
            ...
            raise ActivityFetchError(exc) from exc

    output_df = normalize_activities(df)
    output_df = add_pipeline_metadata(output_df)
    output_df = compute_activity_bounds(output_df, cfg.activity_bounds)
    output_df = apply_activity_annotations(
        output_df,
        action_cfg=enrichment_cfg.action_type,
    ...
    head = [c for c in schema_cols if c in output_df.columns]
    tail = sorted(c for c in output_df.columns if c not in schema_cols)
    col_order = head + tail

    return ActivityFrameResult(
        data=output_df,
...
def write_activity_outputs(
    ...
    *,
    ...
    """Validate and persist activity outputs to disk."""

    df = frame.data
    rows_total = frame.rows_total
    exit_code = 0

    required_cols = {name for name, col in ActivitiesSchema.columns.items() if col.required}
