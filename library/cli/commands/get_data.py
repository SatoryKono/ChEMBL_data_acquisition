...
from library.orchestration.artifacts import (
    PostprocessResult,
    SidecarArtefact,
    cleanup_diagnostics,
    ...
    discover_sidecars,
    finalize_outputs,
    is_diagnostic_sidecar,
    remove_path,
    resolve_postprocess_table,
...
def _is_diagnostic_sidecar(path: Path) -> bool:
    return is_diagnostic_sidecar(path)


def _remove_path(path: Path) -> None:
    remove_path(path, logger=_LOGGER)


def _cleanup_empty_directories(path: Path, *, root: Path) -> None:
    cleanup_empty_directories(path, root=root)
...
def _resolve_postprocess_table(step: PipelineStep, final_output: Path) -> str | None:
    return resolve_postprocess_table(step, final_output)
