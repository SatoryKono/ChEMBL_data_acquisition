"""Configuration data models and validation rules."""
...
class OpenAlexCfg(_BaseModel):
    ...
    rps: int = Field(5, ge=1)
    burst: int = Field(5, ge=1)
    user_agent: str = "chembl-da/1.0"
    mailto: str = Field(
        "chembl-data@ebi.ac.uk",
        description="Contact email passed to OpenAlex",
    )
...
class CrossRefCfg(_BaseModel):
    ...
    mailto: str = Field(
        "chembl-data@ebi.ac.uk",
        description="Contact email passed to CrossRef",
    )
...
class ChemblSourceCfg(_BaseModel):
    ...
    pipelines: ChemblPipelinesRootCfg = Field(
        default_factory=lambda: ChemblPipelinesRootCfg()
    )
...
class IupharCfg(_BaseModel):
    base: str = "https://www.guidetopharmacology.org"
    timeout_connect: float = Field(5.0, ge=1)
    timeout_read: float = Field(30.0, ge=1)
    retries: int = Field(3, ge=0)
    backoff_factor: float = Field(0.5, ge=0)
...
class PubChemCfg(_BaseModel):
    ...
    user_agent: str = Field(
        ...
        description="User agent sent with PubChem requests",
    )
    mailto: str = Field(
        "chembl-data@ebi.ac.uk",
        description="Contact email passed to PubChem",
    )
    api_key: str | None = Field(
        default=None,
        description="Optional PubChem API key",
    )
    enable_caching: bool = Field(
        True,
        description="Enable HTTP cache for PubChem responses",
    )
    cache_backend: str = Field(
        "sqlite",
        description="HTTP cache backend",
    )
    cache_name: str = Field(
        "pubchem",
        description="HTTP cache namespace",
    )
    cache_expire_after: int | None = Field(
        3600,
        ge=1,
        description="Cache TTL in seconds",
    )
    cache_allowable_codes: tuple[int, ...] = Field(
        default_factory=lambda: (200, 202),
        description="HTTP codes that are cached",
    )
    cache_ignored_parameters: tuple[str, ...] = Field(
        default_factory=lambda: ("api_key",),
        description="Query parameters ignored when caching",
    )
    cache_use_temp: bool = False
    mailto_header: bool = True
    prefer_http_cache: bool = True
...
class ActivityBoundsCfg(_BoolModel):
    ...
    @field_validator("enable_from_relation", "enable_from_uncertainty", mode="before")
    @classmethod
    def _bools(cls, v: Any) -> bool:
        return cls._parse_bool(v)


class ActivityActionTypeCfg(_BaseModel):
    column: str = "action_type"
    summary_column: str = "action_type_summary"
    test_value: str = "positive"
    ...
    @classmethod
    def _valid(cls, v: str, info: Any) -> str:
        choices = info.data.get("valid_values", cls.valid_values)
        if v not in choices:
            raise ValueError(
                f"activity_enrichment.action_type.test_value must be in {choices}, got {v!r}"
            )
        return v
...
class ActivityPropertiesCfg(_BaseModel):
    ...
    summary_column: str = "activity_properties_summary"
    name_field: str = "name"
...
class TestitemMoleculeEnrichmentFlagsCfg(_BaseModel):
    enable_parent_watchdog: bool = True
    parent_watchdog_timeout_minutes: float = Field(10.0, ge=0)
    include_parent: bool = True


class TestitemMoleculeEnrichmentLoggingCfg(_BaseModel):
    ...
    def _level(cls, v: str) -> str:
        ...
        if v.upper() not in valid:
            raise ValueError(
                f"testitem.molecule_enrichment.logging.level must be one of {sorted(valid)}"
            )
        return v
...
class ChemblPipelinesRootCfg(_BaseModel):
    activity: ActivityCfg = Field(default_factory=lambda: ActivityCfg())
    assay: AssayCfg = Field(default_factory=lambda: AssayCfg())
    cellline: CelllineCfg = Field(default_factory=lambda: CelllineCfg())
    tissue: TissueCfg = Field(default_factory=lambda: TissueCfg())
    testitem: TestitemCfg = Field(default_factory=lambda: TestitemCfg())
    document: DocumentCfg = Field(default_factory=lambda: DocumentCfg())
    target: TargetAllCfg = Field(default_factory=lambda: TargetAllCfg())
...
class SourcesCfg(_BaseModel):
    ...
    pipelines: Pipelines = Field(default_factory=lambda: SourcesCfg.Pipelines())
