...
паттерн, поэтому менять или иначе ротировать базовое имя файла нельзя. Легаси
обёртка `python scripts/get_data.py` использует те же хелперы, сохраняя при этом
исторический суффикс с точностью до секунд: каждый запуск создаёт
`logs/get_data_<YYYYMMDD_HHMMSS>.log` и прокидывает этот таймстамп в качестве
`run_id` для downstream-отчётности.
