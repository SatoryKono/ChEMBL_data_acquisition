...
Успешный прогон завершится с кодом `0`; нарушения порога успешности или
...
`data/logs/<program>_<YYYYMMDD>.log`, где `<program>` — имя скрипта (например,
`get_data` → `data/logs/get_data_20250228.log`). Если CLI принимает `--base-path`,
значение разворачивается (`~` → домашний каталог), переводится в абсолютный путь и
к нему добавляется `logs/` (то есть `--base-path ../exports` даст
`<абсолютный-exports>/logs/get_data_<YYYYMMDD>.log`). Команды, которые используют
путь по умолчанию, учитывают переменную среды `CHEMBL_DA_BASE_PATH`, применяя те же
правила нормализации, но интерпретируя относительные значения относительно корня
репозитория перед добавлением `logs/`. В обоих случаях сохраняется схема
`<program>_<YYYYMMDD>.log`, причём явный каталог из CLI имеет приоритет над
переменной окружения. Тесты и операционные регламенты опираются на этот паттерн,
поэтому менять или иначе ротировать базовое имя файла нельзя.
