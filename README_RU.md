...
Ниже приведены примеры запуска основных CLI-инструментов с типовыми флагами (`--input`, `--output`, `--limit`). Параметр `--limit 0` допустим: после проверки аргументов пайплайн завершится без обработки, что удобно для быстрых smoke-тестов конфигурации.
