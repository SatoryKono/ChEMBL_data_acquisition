...
* Унифицированные CLI-флаги `--input`, `--output`/`--out`, `--final-out`, `--log-level`, `--sep`, `--encoding`, `--column`, а
  также `--config` и `--print-config` для управления конфигурацией. Зарезервированные `--raw-out`/`--raw-format` реализованы
  пока только в таргет-пайплайне.
...
3. **Изучите установленные консольные скрипты**
  ...
  get-activity-data --input tests/data/activity_ids_small.csv \
      --out out/activities.final.csv --limit 10 --log-level INFO
...
Ниже приведены примеры запуска основных CLI-инструментов с типовыми флагами (`--input`, `--out`/`--output`, `--limit`). Параметр `--limit 0` допустим: пайплайн завершится до любых сетевых и файловых операций, что удобно для быстрых smoke-тестов конфигурации.
...
Обновлённая конвейерная обработка резервирует отдельные точки вывода для «сырого» и нормализованного результата. На текущий
момент реализация доступна только в таргет-пайплайне: `--raw-out` сохраняет исходный ответ (поддерживается переключение
формата через `--raw-format` между `csv` и `parquet`), а `--final-out`/`--out` управляет финальным экспортом. Остальные
конвейеры принимают эти флаги для совместимости, но пока их игнорируют. Для составных ключей укажите столбцы через
`--id-cols`, чтобы сохранить их в «сыром» снимке после появления поддержки в соответствующих адаптерах.
...
* **Raw CSV / Parquet** — (только таргет-пайплайн) сохранение необработанного ответа в `--raw-out` с заданным `--raw-format`.
...
Если `--raw-out` не указан — или пайплайн ещё не поддерживает сохранение «сырого» слоя — этап пропускается. Sidecar-файлы
по-прежнему содержат параметры запуска, конфигурационные диффы и хеши независимо от выбранных форматов. Расширение поддержки
«сырого» вывода на остальные конвейеры запланировано отдельно.
...
В таргет-пайплайне используйте `--raw-out` (при необходимости с `--raw-format`) и `--final-out`, чтобы развести «сырой» и чистый
вывод. Дополнительные примеры приведены в [`docs/USAGE_RU.md`](docs/USAGE_RU.md) (английская версия —
...
По умолчанию `--output`/`--out` формируется как `output.<имя_входа>_YYYYMMDD.csv` в каталоге `local.io.output_dir`. В таргет-
пайплайне добавьте `--raw-out`/`--raw-format` и `--final-out`, чтобы явно развести «сырые» и чистые артефакты. Дополнительные
примеры см. в [`docs/USAGE_RU.md`](docs/USAGE_RU.md) (английская версия — [`docs/USAGE_EN.md`](docs/USAGE_EN.md)).
