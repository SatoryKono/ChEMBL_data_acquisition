...
5. В средах только для чтения используйте
   `python scripts/check_determinism.py --dry-run`. Скрипт сперва убеждается,
   что оба прогона не создали CSV-артефактов; при их наличии проверка падает.
   Затем он хэширует объединённые stdout/stderr двух последовательных запусков
   и выводит `Dry-run log hash check: matched`, если план выполнения
   детерминирован.
