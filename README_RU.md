...
`pre-commit` выполняет форматирование, линтинг и статические проверки. Для юнит-тестов используйте `pytest`, при необходимости добавляйте параметры покрытия. Дополнительные smoke- и детерминизм-проверки доступны через отдельные CLI. Актуальный список проверок опубликован в документах по QA:
