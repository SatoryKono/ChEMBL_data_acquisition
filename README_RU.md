...
| Tissue | `python scripts/get_tissue_data.py --input data/input/tissue.csv --final-out output/tissues.csv --batch-size 25 --limit 100 --offset 10 --timeout 45` | Получает метаданные тканей из эндпоинта ChEMBL `/tissue`, поддерживает управление размером батчей/ограничениями и формирует детерминированный справочник. |
