...
- `scripts/run_tests.py` завершаетcя с кодом `0` при успехе, с `1` при нарушении
  порогов успешности/покрытия и с `11` при ошибках генерации/валидации
  JSON/Markdown-отчётов; отображайте эти коды в CI для быстрого триажа. Сырой
  отчёт pytest сохраняется отдельно (`reports/pytest_raw_report.json`), а перед
  записью путей из `--json`/`--markdown` обёртка очищает каталоги, поэтому
  перенаправляйте выводы в выделенные директории.
